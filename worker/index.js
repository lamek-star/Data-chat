  ...
  async fetch(request, env) {
    ...
    if (
      ...
      (url.pathname === "/downloads/DataChat-latest.apk" ||
        ...
        url.pathname === "/downloads/DataChat-1.3.4.apk")
    ) {
    ...
    if (
      (request.method === "GET" || request.method === "HEAD") &&
      url.pathname === "/downloads/DataChat-1.3.4.aab"
    ) {
      const bundle = await env.DOWNLOADS?.get("DataChat-1.3.4.aab");
      if (!bundle)
      ...
      headers.set(
        "Content-Disposition",
        'attachment; filename="DataChat-1.3.4.aab"',
      );
